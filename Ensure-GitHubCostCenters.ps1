...
try {
    ...
    try {
        ...
        # Special handling for 404 errors - endpoint might not exist or no cost centers
        if ($_.Exception.Message -match "404") {
            Write-ColorOutput "Cost Centers API endpoint not found (404)" -Color Yellow -Level "WARN"
            Write-ColorOutput "This may indicate:" -Color Yellow -Level "WARN"
            Write-ColorOutput "  - No cost centers exist yet (safe to proceed)" -Color Yellow -Level "WARN"
            Write-ColorOutput "  - Cost Centers feature not available in your GitHub plan" -Color Yellow -Level "WARN"
            Write-ColorOutput "`nProceeding with assumption that no Cost Centers exist..." -Color Yellow -Level "WARN"
            $existingCostCenters = @()
        }
        else {
            # For other errors, fail safely
            Write-ColorOutput "✗ Failed to retrieve existing Cost Centers" -Color Red -Level "ERROR"
            ...
            throw "Failed to retrieve existing Cost Centers: $_"
        }
    }
    }
